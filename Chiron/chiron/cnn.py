...
def simple_global_bn(inp, name):
    ...
    #return tf.nn.batch_normalization(inp, mean=mean, variance=variance, scale=scale, offset=offset,
                                     #variance_epsilon=1e-5, training = training)
    return tf.contrib.layers.batch_norm(inp, scale=scale)
...
#########################################################
#########################################################
############## START MAJOR PROJECT EDITS ################
#########################################################
#########################################################

## ResNext Model Information:
# Paper: https://arxiv.org/pdf/1611.05431.pdf
# Base Inplementation: https://github.com/taki0112/ResNeXt-Tensorflow

def DNA_model2(net,training):
    #   Residual Layer x 3 (DNA_default)
    print ("\n\nIn MODEL2!!!\n\n")

    with tf.variable_scope('res_layer1'):
        net = first_layer(net, scope='res_layer1', training = training)
        net = resnext_residual_layer(net, out_dim=64, layer_num='1', training = training)

    print ("ONE LAYER")
    print (net)
    with tf.variable_scope('res_layer2'):
        net = resnext_residual_layer(net, out_dim=128, layer_num='2', training = training)

    print ("TWO LAYER")
    print (net)
    with tf.variable_scope('res_layer3'):
        net = resnext_residual_layer(net, out_dim=256, layer_num='3', training = training)
...
def resnext_residual_layer(input_x, out_dim, layer_num, training, res_block=3):
...
def split_layer(input_x, stride, layer_name, training):
...
def transition_layer(x, out_dim, scope):
    fea_shape = x.get_shape().as_list()
    in_channel = fea_shape[-1]
    with tf.name_scope(scope):
        x = conv_layer2(x, filter=out_dim, kernel=[1,1], stride=1, layer_name=scope+'_conv1')
        x = simple_global_bn(x, name=scope + 'conv11_bn')
...
def first_layer(x, scope, training):
    with tf.name_scope(scope) :
        x = conv_layer2(x, filter=64, kernel=[3, 3], stride=1, layer_name=scope+'_conv1')
        x = simple_global_bn(x, name=scope + 'conv12_bn')
        x = Relu(x)
    return x

def transform_layer(x, stride, scope, training):
    with tf.name_scope(scope) :
        x = conv_layer2(x, filter=depth, kernel=[1,1], stride=stride, layer_name=scope+'_conv1')
        x = simple_global_bn(x, name=scope + 'conv13_bn')
        x = Relu(x)

        x = conv_layer2(x, filter=depth, kernel=[3,3], stride=1, layer_name=scope+'_conv2')
        x = simple_global_bn(x, name=scope + 'conv2_bn')
        x = Relu(x)
    return x

def conv_layer2(input, filter, kernel, stride, padding='SAME', layer_name="conv2"):
    with tf.name_scope(layer_name):
